...
@implementation AbstractView
    ...
    - (void)resizeWithFrame:(NSRect)frame parsedMasks:(NSArray *)parsedMasks {
        ...
        if (self.nsView && [self.nsView.layer isKindOfClass:[IOSurfaceLayer class]]) {
            IOSurfaceLayer *layer = (IOSurfaceLayer *)self.nsView.layer;
            CGFloat scale = self.nsView.window.backingScaleFactor;
            CGFloat oldScale = layer.contentsScale;
